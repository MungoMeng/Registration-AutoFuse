...
class AutoFuse(nn.Module):
    ...
    def __init__(self,
        ...
        self.FusionPath = FusionPath(in_channels=in_channels*2,
    ...
    def forward(self, fix_vol, mov_vol):
        
        fix_feat = self.SinglePath(fix_vol)
        mov_feat = self.SinglePath(mov_vol)
        SVF_mean, SVF_log_sigma = self.FusionPath(fix_vol, mov_vol, fix_feat, mov_feat)
        
        SVF = self.Sample(SVF_mean, SVF_log_sigma)
        flow = self.Integration(SVF)
        
        flow = self.ResizeTransformer(flow)
        warp_vol = self.SpatialTransformer(mov_vol, flow)
        
        SVF_params = torch.cat([SVF_mean, SVF_log_sigma], dim=1)
        return flow, SVF_params, warp_vol
...
class AutoFuse_Trans(nn.Module):
    ...
    def __init__(self, 
        ...
        self.SinglePath = SinglePath_Trans(in_channels=in_channels,
                                           channel_num=channel_num,
                                           use_checkpoint=use_checkpoint)
        self.FusionPath = FusionPath_Trans(in_channels=in_channels*2,
                                           channel_num=channel_num*2,
                                           use_checkpoint=use_checkpoint)
...
class SinglePath(nn.Module):
    ...
    def forward(self, x_in):
        ...
        return [x_2, x_3, x_4, x_5, x_6, x_7, x_8]
...
class FusionPath(nn.Module):
    ...
    def forward(self, fix_vol, mov_vol, fix_feat, mov_feat):
        
        x_in = torch.cat([fix_vol, mov_vol], dim=1)
        x_1 = self.conv_1(x_in)
