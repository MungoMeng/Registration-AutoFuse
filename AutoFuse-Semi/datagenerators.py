...
def gen_s2s(gen, batch_size=1):
    ...
    while True:
        ...
        yield ([fix_vol, mov_vol, mov_seg], [Zero, Zero, fix_vol, fix_seg, mov_seg, fix_seg, fix_seg, Zero])
...
def gen_pairs(path, pairs, batch_size=1):
    ...
    while True:
        ...
        for idx in idxes:
            fixed = bytes.decode(pairs[idx][0])
            X = load_volfile(path+fixed, np_var='all')
            if 'seg' in X.files:
                X = X['seg']
            else:
                X = np.zeros(X['vol'].shape)
            X = X[np.newaxis, np.newaxis, ...]
            X_data.append(X)
        if batch_size > 1:
        ...
        for idx in idxes:
            moving = bytes.decode(pairs[idx][1])
            X = load_volfile(path+moving, np_var='all')
            if 'seg' in X.files:
                X = X['seg']
            else:
                X = np.zeros(X['vol'].shape)
